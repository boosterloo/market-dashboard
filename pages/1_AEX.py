...
def heikin_ashi(src: pd.DataFrame):
    ...
    ha_open = pd.Series(index=src.index)
...
# Download
st.download_button("Download CSV", d.to_csv(index=False).encode('utf-8'), "aex_data.csv")

if "vix_close" in last and last["vix_close"] > DEFAULTS["vix_high"]:
    st.error("High VIX: Verhoogde volatiliteit!")
if d["rsi14"].iloc[-1] < DEFAULTS["rsi_os"]:
    st.warning("RSI oversold!")

# ---------- Top-UI ----------
c_top1, c_top2 = st.columns(2)
with c_top1:
    show_vix = st.checkbox("Toon VIX (paneel 1)", value=True, help="VIX op 2e y-as in de bovenste grafiek.")
...
fig3.update_layout(title="Cumulative Returns: Buy-Hold vs Signals", yaxis_title="Cum. Return (%)", height=400)
...
if heat_on:
    ...
    heat.add_trace(go.Bar(x=pivot.columns, y=monthly_avg, name="Gem. per maand", yaxis="y2"))
    heat.update_layout(height=400, margin=dict(l=20, r=20, t=30, b=20),
                       coloraxis=dict(colorscale="RdBu", cauto=True, colorbar_title=value_title),
                       xaxis_title="Maand", yaxis_title="Jaar", title=f"Heatmap — {value_title} + seizoensgemiddelde")
