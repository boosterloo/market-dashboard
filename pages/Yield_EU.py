# pages/Yield_EU.py — EU op nieuwe enriched view, met 1d/7d/30d histograms (bp & %)
...
st.set_page_config(page_title="Yield Curve Dashboard (EU)", layout="wide")
st.title("🇪🇺 EU Yield Curve Dashboard")

PROJECT_ID = st.secrets["gcp_service_account"]["project_id"]
TABLES     = st.secrets.get("tables", {})
DEFAULT_VIEW = f"{PROJECT_ID}.marketdata.eu_yield_curve_enriched_v"   # <- EU ENRICHED VIEW
YIELD_VIEW   = TABLES.get("yield_view", DEFAULT_VIEW)

# ---------- Recessieperioden (NBER; optioneel overlay, ook al is dit EU) ----------
NBER_RECESSIONS = [
...
with cD:
    # Voor EU standaard uit, want NBER is US — kan je aanzetten als referentie
    show_recessions = st.toggle("US recessies (NBER)", value=False)
...
# Beschikbare horizons: 1d (bp beschikbaar of fallback), 7d/30d (pp → *100 naar bp)
...
if available_horizons:
    ...
    if candidates:
        ...
        pick_base, pick_col = st.selectbox(
            ...
            candidates,
            index=def_idx,
            format_func=lambda t: labels_map.get(t, t[0]),
            key="hist_metric",
        )
...
st.download_button("⬇️ Download CSV (gefilterd op periode)", data=csv,
                   file_name="yield_curve_filtered_eu.csv", mime="text/csv")
