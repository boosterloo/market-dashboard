...
# Supertrend op Heikin‑Ashi (10,1)
def supertrend_on_ha(ha: pd.DataFrame, length: int = 10, multiplier: float = 1.0):
    ...
    final_upper = np.zeros(len(ha)); final_lower = np.zeros(len(ha))
    trend = np.ones(len(ha))  # +1 up, -1 down
    final_upper[:] = np.nan;  final_lower[:] = np.nan
    ...
    st_line = pd.Series(np.where(trend == 1, final_lower, final_upper),
                        index=ha.index, name="st_line")
...
ha = heikin_ashi(d).reset_index(drop=True)
...
for col in ["ha_open","ha_high","ha_low","ha_close"]:
    d[col] = ha[col]
d["st_line"] = st_df["st_line"]
d["st_trend"] = st_df["trend"]   # +1/-1
...
# (1) HA + DC + ST (groen/rood als segmenten)
