...
def pick_closest_value(options: list[float], target: float, fallback: float | None = None) -> float | None:
    if not options:
        return fallback
...
# Slimme default strike:
#  - PUT: onderliggende − 300
#  - CALL: onderliggende + 200
#  - Score = 1.0 * norm(volume) + 0.5 * norm(oi) + 0.25 * round_bonus − 0.15 * norm(|strike − target|)
strikes_all = sorted([float(x) for x in df["strike"].dropna().unique().tolist()])
...
# binning per 0.25%-punt, medianen, min. 5 waarnemingen per bin
...
# binning in DTE (stap=1) met medianen en smoothing
...
# ── E) Matrix — meetmoment × strike ──────────────────────────────
st.subheader("Matrix — meetmoment × strike")
colM1, colM2, colM3, _ = st.columns([1, 1, 1, 1])
with colM1:
    matrix_exp = st.selectbox("Expiratie (matrix)", options=sorted(df["expiration"].unique().tolist()), index=0, key="mx_exp")
with colM2:
    matrix_metric = st.radio("Waarde", ["last_price", "mid_price", "ppd"], horizontal=False, index=0, key="mx_metric")
...
# IV Smile (laatste snapshot) per gekozen expiratie
...
# Put/Call ratio per expiratie (volume & OI)
...
# OI per strike (top 5 expiraties)
st.subheader("Open Interest per Strike (top-5 expiraties)")
if exps:
    for e in exps[:5]:
        sub = df[df["expiration"] == e].groupby("strike", as_index=False)["open_interest"].sum().sort_values("strike")
        if not sub.empty:
            fig = go.Figure(go.Bar(x=sub["strike"], y=sub["open_interest"]))
            fig.update_layout(title=f"Open Interest per Strike — {sel_type.upper()} exp {e}",
                              xaxis_title="Strike", yaxis_title="Open Interest", height=320)
            st.plotly_chart(fig, use_container_width=True)
