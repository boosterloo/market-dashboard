...
with col_out1:
    outlier_mode = st.radio("Outlier-methode", ["Geen", "Percentiel clip", "IQR filter", "Z‑score filter"], horizontal=True, index=1)
with col_out2:
...
if serie.empty:
    ...
    fig_ser.add_trace(go.Scatter(x=serie["snapshot_date"], y=apply_outlier(serie[series_price_col], outlier_mode, pct_clip), name="Price", mode="lines+markers"), secondary_y=False)
    fig_ser.add_trace(go.Scatter(x=serie["snapshot_date"], y=apply_outlier(serie["ppd"], outlier_mode, pct_clip), name="PPD", mode="lines"), secondary_y=True)
    ...
    if outlier_mode != "Geen":
        fig_ser.add_annotation(xref="paper", yref="paper", x=1, y=1.08, xanchor="right", showarrow=False, text=f"Outlier: {outlier_mode}")
...
sel_snapshot = st.selectbox("Peildatum (snapshot)", options=snapshots, index=default_idx, format_func=lambda x: pd.to_datetime(x).strftime("%Y-%m-%d %H:%M"))
df_last = df[df["snapshot_date"] == sel_snapshot].copy()

ppd_vs_dist = (
    df_last.assign(ppd_f=apply_outlier(df_last["ppd"], outlier_mode, pct_clip))
          .groupby(((df_last["dist_points"].abs() / df_last["underlying_price"]) * 100.0).round(2), as_index=False)["ppd_f"].mean()
...
fig_ppd_dist.update_layout(title=f"PPD vs Afstand tot Strike — {pd.to_datetime(sel_snapshot).strftime('%Y-%m-%d %H:%M')}", xaxis_title="Afstand tot strike (|strike − underlying| / underlying, %)", yaxis_title="PPD", height=400)
...
fig_exp.update_layout(title=f"{sel_type.upper()} — Strike {series_strike} — peildatum {pd.to_datetime(sel_snapshot).strftime('%Y-%m-%d %H:%M')}", height=420, hovermode="x unified")
...
if outlier_mode != "Geen":
    fig_exp.add_annotation(xref="paper", yref="paper", x=1, y=1.08, xanchor="right", showarrow=False, text=f"Outlier: {outlier_mode}")
...
if ppd_mode.startswith("ATM"):
    ...
    df_ppd = df[(df["strike"] >= series_strike - strike_window) & (df["strike"] <= series_strike + strike_window)].copy()
...
if mx.empty:
    ...
    with tab_hm:
        ...
        fig_mx = go.Figure(data=go.Heatmap(z=z, x=x, y=y, colorbar_title=matrix_metric.capitalize(),
                                           hovertemplate="Snapshot: %{y}<br>Strike: %{x}<br>Value: %{z:.2f}<extra></extra>"))
        fig_mx.update_layout(title=f"Heatmap — {sel_type.upper()} exp {matrix_exp} — {matrix_metric}", xaxis_title="Strike", yaxis_title="Meetmoment", height=520)
