...
colmap = {
    ...
    # PPI (varianten + mogelijke mislabel)
...
# UI-fallback voor PPI (bijv. als PPI in de view 'ind_production' heet)
...
# 1) Start met wat er nu in dfp aanwezig is
act_cols = [c for c in ["industrial_production","retail_sales","housing_starts"] if c in dfp.columns]

# 2) Fallback: als 'industrial_production' ontbreekt maar bronkolom bestaat en NIET als PPI gebruikt is
...
# ========= Sectie: Arbeidsmarkt (payrolls op y2) =========
    ...
    if view_mode.startswith("Genormaliseerd"):
        ...
        if right_col:
            ...
            fig3.add_trace(go.Scatter(
                ...
                line=dict(width=3, dash="dash", color=PALETTE[3])  # groen, duidelijk zichtbaar
            ))
