# pages/Yield.py — US op nieuwe enriched view, met 1d/7d/30d histograms (bp & %)
...
with sigR:
    # Regime-keuze op 7d/30d (zoals afgesproken)
...
# ================== Δ DELTAS — eerst HISTOGRAMMEN (bp & %) ==================
...
if available_horizons:
    ...
    if candidates:
        # default 10Y indien beschikbaar
        def_idx = 0
...
if available_horizons:
    ...
    if choose:
        ...
        for c in choose:
            if suf2 == "_d1_bp":
                # al in bp
                yvals_bp = pd.to_numeric(df_range[c], errors="coerce")
            else:
                # pp → bp
                yvals_bp = pd.to_numeric(df_range[c], errors="coerce") * 100.0
