...
# App setup
# =========================
st.set_page_config(page_title="📊 AEX — Signals, Windows & Backtest", layout="wide")
st.title("📊 AEX — Signals, Event Windows (🟨), Options-proxy & Advanced Backtest")
...
numeric_cols = ["open","high","low","close","vix_close","delta_abs","delta_pct"]
for c in numeric_cols:
    if c in df.columns:
        df[c] = pd.to_numeric(df[c], errors="coerce")

# Detectie VIX-aanwezigheid
has_vix = ("vix_close" in df.columns) and df["vix_close"].notna().any()
...
with st.sidebar:
    ...
    # Toggle voor plotten VIX + z-score optie; disable als VIX ontbreekt
    show_vix  = st.toggle("Toon VIX in paneel 1", value=True if has_vix else False, disabled=not has_vix)
    vix_as_z  = st.checkbox("VIX als z-score plotten", value=False, disabled=not has_vix)
    corr_vs   = st.radio("Rolling correlatie vs VIX", ["% change", "level"], index=0, disabled=not has_vix)
    ...
    window_rule = st.selectbox("Window-regel", ["VIX spike", "Supertrend flip", "Donchian breakout", "UNION (alles)"], index=0,
                               help="‘VIX spike’ wordt automatisch uitgeschakeld als VIX ontbreekt.")
...
# VIX spike mask (veilig wanneer VIX ontbreekt)
if has_vix:
    ...
    mask_vix = pd.Series(False, index=d.index)
...
# Als VIX ontbreekt en user koos "VIX spike", val terug naar UNION zonder VIX
chosen_rule = window_rule
if (not has_vix) and (window_rule == "VIX spike"):
    chosen_rule = "UNION (alles)"

if chosen_rule == "VIX spike":
    win_list = _windows_from_bool(mask_vix, min_len=min_len, pad=flip_pad)
elif chosen_rule == "Supertrend flip":
    win_list = _windows_from_bool(mask_flip, min_len=min_len, pad=flip_pad)
elif chosen_rule == "Donchian breakout":
...
k3.metric("VIX (close)", f"{last['vix_close']:.2f}" if has_vix and pd.notna(last.get("vix_close", np.nan)) else "—")
...
# =========================
alerts = []
if has_vix and pd.notna(last.get("vix_close", np.nan)):
...
# =========================
# Paneel 1 – HA + Supertrend + Donchian (+ VIX) + 🟨 windows
# =========================
vix_yaxis_title = "VIX z" if (has_vix and st.session_state.get("__vix_as_z__", vix_as_z)) else "VIX"

fig1 = make_subplots(rows=1, cols=1, specs=[[{"secondary_y": True}]],
                     subplot_titles=[f"AEX Heikin-Ashi + Supertrend (10,1) + Donchian" + ((" + VIX (2e y-as)" if has_vix and show_vix else ""))])
...
# VIX-plot (raw of z-score)
if has_vix and show_vix:
    vix_ser = d["vix_close"]
    if vix_as_z:
        vix_ma = vix_ser.rolling(20, min_periods=20).mean()
        vix_sd = vix_ser.rolling(20, min_periods=20).std()
        vix_ser = (vix_ser - vix_ma) / vix_sd
        vix_name = "VIX z-score (sec. y)"
    else:
        vix_name = "VIX (sec. y)"
    fig1.add_trace(go.Scatter(x=d["date"], y=vix_ser, mode="lines", name=vix_name, line=dict(width=2)),
...
fig1.update_layout(
    ...
    yaxis2=dict(title=(vix_yaxis_title if has_vix and show_vix else ""), tickfont=dict(size=13))
)
...
if has_vix:
    ...
    st.info("VIX ontbreekt in de dataset — rolling correlatie niet beschikbaar.")
...
valid["straddle_pnl"] = valid["abs_move"] - (st.session_state.get("__straddle_cost_atr__", None) or 0)  # placeholder; overschreven hieronder
# Short Strangle proxy:
# PnL = premium×ATR − max(0, |Δ| − width×ATR)
overflow = (valid["abs_move"] - 0).clip(lower=0.0)  # placeholder; overschreven hieronder

# Gebruik de waarden uit de sidebar (reeds gedefinieerd)
straddle_cost_atr = st.session_state.get("straddle_cost_atr", None)
strangle_prem_atr = st.session_state.get("strangle_prem_atr", None)
strangle_width_atr = st.session_state.get("strangle_width_atr", None)

# Recompute met juiste parameters
valid["straddle_pnl"] = valid["abs_move"] - (st.session_state.get("straddle_cost_atr", 1.0) * valid["atr14"])
overflow = (valid["abs_move"] - st.session_state.get("strangle_width_atr", 1.5) * valid["atr14"]).clip(lower=0.0)
valid["strangle_pnl"] = (st.session_state.get("strangle_prem_atr", 0.6) * valid["atr14"]) - overflow
